...
- **`vm_print_info.sh` requires a running pod**: it queries the UVM via `shimdiag exec (shimdiag list)[0] ...` to dump `uname`, `cpuinfo`, `snp-report`. With no pod running on the VM, `shimdiag list` is empty and the `[0]` index crashes. Always sequence it AFTER the first `c-aci-testing vm runc` (or any other step that leaves a pod resident). The example workflow above puts it after `Run container on VM` for this reason.
