...
test_types = [
    TestType("basic-region", 20, 6, 1),
    TestType("region", 60, 6, 2),
    TestType("uptime", 5, 20, 3),
    TestType("high-spec", 5, 15, 4),
    ...
    TestType("vn2", 20, 15, 6),

    TestType("perf", 120, 20, 10),
]
...
for wf, t in sorted(tests, key=lambda x: x[1].priority):
    ...
    with open(os.path.join(workflows_dir, wf), "rt") as f:
        content = f.read()
    found_corn = CRON_RE.search(content)
    if not found_corn:
        print(f"Failed to replace corn in {wf}")
        sys.exit(1)
    str_span = found_corn.span(1)
    orig_len = str_span[1] - str_span[0]
    comment_span = found_corn.span(2)
    ...
    with open(os.path.join(workflows_dir, wf), "wt") as f:
        f.write(content)
